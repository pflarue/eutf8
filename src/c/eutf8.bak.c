...
size_t from_eutf8(char* dstdata, int dstsize, char* srcdata, int srclen) {
	...
	while (didx < dstsize) {
		...
		if (srclen > offset + 5
			&& srcdata[offset + 1] == 0xE2
			&& srcdata[offset + 2] == 0x80
			&& srcdata[offset + 3] == 0x8B
		) {
			...
			while (
				...
				&& srcdata[offset + 3 * zws_cnt + 1] == 0xE2
				&& srcdata[offset + 3 * zws_cnt + 2] == 0x80
				&& srcdata[offset + 3 * zws_cnt + 3] == 0x8B
			)
				zws_cnt += 1;
			if (
...
bool escbytes(char *dstdata, int *didx, char *srcdata, int *sidx,
	...
	for (int i = 0; i < count; ++i) {
		...
		char high = srcdata[*sidx] >> 4;
		if (high <= 9)
			high += '0';
		else
			high = 'A' + (high - 10);
		char low = srcdata[*sidx] & 0x0F;
		if (low <= 9)
			low += '0';
		else
			low = 'A' + (low - 10);
...
// indata will be converted to '\\09' in dstdata.  Otherwise, this character in
// in srcdata will be left unchanged in dstdata.
//
// If escape_lf is true, then an ascii line feed ('\n' or '\x0A') in indata will
// be converted to '\\0A' in dstdata.  Otherwise, this character in srcdata will
// be left unchanged in dstdata.
//
// If escape_cr is true, then an ascii carriage return ('\r' or '\x0D') in
// indata will be converted to '\\0D' in dstdata.  Otherwise, this character in
...
size_t to_eutf8(char* dstdata, int dstsize, char* srcdata, int srclen,
	...
	while (sidx < srclen && didx < dstsize) {
			...
			if (c == 0x5C) {
				...
				*/
				...
				while (
					...
					&& srcdata[sidx + 3 * zws_cnt + 1] == 0xE2
					&& srcdata[sidx + 3 * zws_cnt + 2] == 0x80
					&& srcdata[sidx + 3 * zws_cnt + 3] == 0x8B
				)
					zws_cnt += 1;
				if (
					...
					&& srcdata[sidx + 3 * zws_cnt + 1] != 0x00
					&& srcdata[sidx + 3 * zws_cnt + 2] != 0x00
		...
		if (srclen <= sidx + 2) {
			// srcdata[sidx] may be the first byte of a multi-byte UTF-8
			// sequence, but the required number of additional bytes are not
			// available.
			// Escape 1 byte
			if (!escbytes(dstdata, &didx, srcdata, &sidx, 1, dstsize))
				break;
